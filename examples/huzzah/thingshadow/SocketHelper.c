...
char *ICACHE_FLASH_ATTR getCertsDir(char *certsDir, size_t maxCertsDirLength)
{
  ...
  {
    os_snprintf(certsDir, maxCertsDirLength, "%s%s", workspaceDir, "/code/certs");
  }
  else
  {
    os_strncpy(certsDir, "./certs", maxCertsDirLength);
  }
  return certsDir;
}
...
void ICACHE_FLASH_ATTR ipv4ToStr(uint32_t ipAddr, char *ipAddrStr, size_t ipAddrStrSize)
{
  ...
  os_sprintf(ipAddrStr, "%d.%d.%d.%d", ((uint8_t)(((ipAddr >> 24) & 0xFFu))), ((uint8_t)(((ipAddr >> 16) & 0xFFu))), ((uint8_t)(((ipAddr >> 8) & 0xFFu))), ((uint8_t)((ipAddr & 0xFFu))));
}
...
uint32_t ICACHE_FLASH_ATTR strToIPv4(char *ipAddrStr, size_t ipAddrStrLength)
{
  ...
  os_strncpy(tmpStr, ipAddrStr, ipAddrStrLength);
