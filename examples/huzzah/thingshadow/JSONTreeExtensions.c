...
static struct jsontree_value *ICACHE_FLASH_ATTR jsontree_next_array_item_value(struct jsontree_context *js_ctx);

static void ICACHE_FLASH_ATTR jsontree_enter_value(struct jsontree_context *js_ctx, struct jsontree_value *value);

static struct jsontree_value *ICACHE_FLASH_ATTR jsontree_exit_value(struct jsontree_context *js_ctx);

static struct jsontree_value *ICACHE_FLASH_ATTR jsontree_find_pair_value(struct jsontree_context *js_ctx, char const *pairName, size_t pairNameLength);

static bool ICACHE_FLASH_ATTR jsonparse_do_parse_value(struct jsontree_context *js_ctx, struct jsonparse_state *parseState);

bool ICACHE_FLASH_ATTR jsontree_print_path(struct jsontree_context *js_ctx, struct jsontree_msg *msg, char const *path)
{
  ...
  do {
    nextSegment = os_strstr(segment, ((char *)(JSON_OBJECT_PATH_SEPARATOR)));
    ...
    if (length > 0) 
    {
      ...
      else
      {
        ...
        for ( int16_t __i = 0 ; __i < object->count; __i++ )
        {
          ...
          if (length == os_strlen(attributeName) && os_strncmp(segment, attributeName, length) == 0) 
          {
  ...
  while (nextSegment != NULL && os_strlen(nextSegment) > 0 && value != NULL);
  
  js_ctx->path = js_ctx->depth;
  return value != NULL;
}

bool ICACHE_FLASH_ATTR jsontree_print_current_attribute_name(struct jsontree_context *js_ctx, struct jsontree_msg *msg)
{
  struct jsontree_value *value = js_ctx->values[js_ctx->depth];
  if (value->type != JSON_TYPE_OBJECT) 
  {
  ...
  jsontree_write_string(msg, name, ((int32_t)(os_strlen(name))));
  jsontree_write_char(msg, ':');
  return true;
}

bool ICACHE_FLASH_ATTR jsontree_print_current_value(struct jsontree_context *js_ctx, struct jsontree_msg *msg, bool modifiedOnly)
{
...
bool ICACHE_FLASH_ATTR jsontree_print_object_endings(struct jsontree_context *js_ctx, struct jsontree_msg *msg, uint8_t count)
{
  /* 
...
static struct jsontree_value *ICACHE_FLASH_ATTR jsontree_find_pair_value(struct jsontree_context *js_ctx, char const *pairName, size_t pairNameLength)
{
  ...
  if (value->type == JSON_TYPE_OBJECT) 
  {
    ...
    for ( int16_t __i = 0 ; __i < object->count; __i++ )
    {
      if (os_strlen(object->pairs[__i].name) == pairNameLength && os_strncmp(object->pairs[__i].name, pairName, pairNameLength) == 0) 
      {
        js_ctx->index[js_ctx->depth] = __i;
...
bool ICACHE_FLASH_ATTR jsonparse_parse_value(struct jsontree_context *js_ctx, struct jsonparse_state *parseState)
{
    ...
    switch (type)
    {
      ...
      case JSON_TYPE_STRING:
      {
        if (value->type == JSON_TYPE_STRING) 
        {
          ...
          string->len = os_strlen(string->value);
        }
